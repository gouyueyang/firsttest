...
class WpEditorTitle extends React.Component{
	render(){
		...
		return(
			<p className="warp">
				<span className="courseName"><span className="title">{courseName}</span>课程分配专家结果</span>
				<span className="teamName">{'所属课程分组名称 : ' + teamName}</span>
			</p>
		);
	}
}
		...
		for( i = 0 ; i < len ; i++){
			result.push(
				<ul className="List" key={i}>
					...
					<li className="course">
						{this.createCourseSpan(items[i].courses)}
					</li>
			...
			<div>
				...
				<div className="ListWarp">
					<ul className="headerLine">
						...
						<li className="master">专家</li>
						<li className="college">工作单位/所属学院</li>
						<li className="course">已分配课程</li>
