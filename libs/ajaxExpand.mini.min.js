var ajaxExpanding=(function () {
	...
	function encodeData(dataObj,name,that,dataType,comma){
		var data='',
			 i,
			 j,
			 isSuffix = true,
			...
			case 'form':// 通过formData 传输
				if(window.FormData){
					...
					if(typeof dataObj == 'object'){
						...
						for( key in dataObj){
							// 文件检测
								if(Object.prototype.toString.call(dataObj[key].value) == '[object FileList]' || dataObj[key].dataType==='file') {
								checkInf = {
									isCheck:true,
									type:dataObj[key].type || key
								}
								len = dataObj[key].value.length;
								if (len == 0 && !dataObj[key].isEmpileFile) {
									that.ischeck=false;
	...
	return {
		...
		send: function (dataInf, name, bindObj) { // 发送数据
			var that = this[name],
				...
				data = encodeData(dataInf.data, name, that,that.dataType , dataInf.comma);
				if(that.ischeck) {
					that.stopFlag = false;
